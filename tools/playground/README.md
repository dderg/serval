...
the real MCU firmware in the mcu-sim, so it exercises the actual host +
...
Two images: the mcu-sim firmware/klippy image for the branch, then this
playground image (Moonraker + Mainsail) layered on top.

```bash
# 1. Build the sim image for the branch under test (compiles MCU firmware +
#    Rust staticlib + klippy). Tags it mcu-sim-<branch>:
bash tools/mcu-sim/run.sh --branch observability

# 2. Build the playground image on top of it:
docker build -t playground \
  --build-arg BASE=mcu-sim-observability:latest \
...
- `tools/mcu-sim/runner.py --serve --data-dir /printer_data` keeps klippy +
