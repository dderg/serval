...
def _write_scan_gcode(path: pathlib.Path, *, direction, speed, runs,
                      min_xy=(25.0, 25.0), max_xy=(275.0, 275.0),
                      count=(20, 20), z=2.0):
    ...
    pts = generate_path(min_x, min_y, max_x, max_y, res_x, res_y,
                        direction, overscan, 0.0, 0.0)
    fr = speed * 60.0
    lines = ["G90",
...
@pytest.mark.parametrize("arc_fit", [False, True],
                         ids=["arc_fit_off", "arc_fit_on"])
