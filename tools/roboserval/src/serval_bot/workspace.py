...
class CredentialedWorkspace:
    ...
    def sync(self, repo: str, branch: str, token: str) -> Path:
        ...
        self._run("git", "checkout", "-B", branch, f"origin/{branch}", cwd=destination, environment=environment)
        self._run("git", "reset", "--hard", f"origin/{branch}", cwd=destination, environment=environment)
    ...
    @staticmethod
    def _run(*command: str, cwd: Path, environment: dict[str, str]) -> None:
