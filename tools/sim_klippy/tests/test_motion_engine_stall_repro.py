...
def test_linear_move_after_set_kinematic_position(sim):
    ...
    Expected: both succeed without "engine_call:" errors. If this
    ...
    assert "engine_call" not in err, (
        f"SET_KINEMATIC_POSITION already failed with engine_call error: {r}"
    )
    ...
    assert "transport closed" not in err, (
        f"REPRO: engine_call: transport closed during linear move\n  result={r}"
    )
    assert "transport timed out" not in err, (
        f"REPRO: engine_call: transport timed out during linear move\n  result={r}"
    )
...
def test_same_direction_jogs_reproduce_slot_pool_exhaustion(sim):
    ...
    # quiescence window (rust/motion-engine/src/planner.rs:53). Otherwise
...
def test_long_move_during_tmc_poll(sim):
    ...
    motion-engine load_curve traffic, this will reproduce the race.
