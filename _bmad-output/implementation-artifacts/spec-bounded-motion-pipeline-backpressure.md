...
- **2026-06-24 — Intake gate corrected from per-axis ring-room to total backlog (homing-crash fix).** Bench homing crashed: `drip cohort 1: floor stalled at 0` because 3 of 4 cohort axes got `queued 0 executed 0`. Root cause: the ring-room intake gate (`staged < free_room + margin`, plus a `holding_ahead` early-out) is *global over known axes*, but pieces for all axes interleave on **one** data channel — so once the head axis filled its ring (or went `StallAhead` with a clock-less cohort participant pinning the horizon at 0), the pump stopped pulling and never reached the pieces of axes queued behind it. A per-axis ring-room intake gate is unachievable on a shared channel without starvation. Fixed: intake bounded by `wants_pieces = total_staged < PUMP_INTAKE_BACKLOG_CAP` (2048) — a *total* that keeps pulling past a full axis to feed the others; the **send** path stays ring-gated (`room()`/`StallFull`). Regression test: `intake_feeds_a_second_axis_even_when_the_first_axis_ring_is_full`.
