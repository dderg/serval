...
- **`--features hw` is opt-in** and builds the IgH (EtherLab) backend on the Pi (`csrc/libecrt_igh.c`, links `-lethercat`; `IGH_DIR`/`IGH_LIB_DIR` default to `/opt/etherlab`). IgH is the only EtherCAT master backend — SOEM was removed. **Linking** happens only on the Pi (libethercat is absent in CI), but `./scripts/ci.sh rust-ethercat-hw` (Linux-only) `cargo check`s the hw binary against a committed CI stub of `ecrt.h` (`csrc/ci-igh/include/ecrt.h`, pointed to via `IGH_DIR`) — so C + Rust compile errors in the hw path are caught in CI without linking. Keep the stub header faithful to the IgH subset `libecrt_igh.c` uses. The `ethercat-rt-stub` binary (no `--features`) additionally serves the socket protocol without the FFI.
