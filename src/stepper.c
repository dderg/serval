...
command_kalico_resonance_buzz(uint32_t *args)
{
    ...
    uint32_t freq_millihz = args[2];
    uint32_t amplitude_nm = args[3];
    uint32_t duration_ms = args[4];
    uint32_t ramp_ms = args[5];
    int32_t rc = runtime_resonance_buzz(
        runtime_handle, axis_mask, sign_mask, freq_millihz,
        amplitude_nm, duration_ms, ramp_ms);
    if (rc != 0)
        shutdown("kalico_resonance_buzz rejected (bad parameters)");
}
DECL_COMMAND(command_kalico_resonance_buzz,
             "kalico_resonance_buzz axis_mask=%c sign_mask=%c freq_millihz=%u"
             " amplitude_nm=%u duration_ms=%u ramp_ms=%u");
