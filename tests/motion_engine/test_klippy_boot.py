...
class TestEngineModule:
    ...
    def test_claim_primary_mcu(self):
        ...
        engine = motion_engine.MotionEngine()
        handle = engine.claim_mcu("mcu", "socket://localhost:3334", 250000)
    ...
    def test_claim_stub_mcus(self):
        ...
        engine = motion_engine.MotionEngine()
        handles = claim_stub_mcus(engine)
    ...
    def test_alloc_queues_for_all_mcus(self):
        ...
        engine = motion_engine.MotionEngine()
        primary = engine.claim_mcu("mcu", "socket://localhost:3334", 250000)
        stubs = claim_stub_mcus(engine)

        q0 = engine.alloc_command_queue(primary)
...
class TestRenodeSim:
    ...
    def test_engine_with_sim_serial_path(self, renode_sim):
        import motion_engine
        ...
        q = engine.alloc_command_queue(handle)
        engine.passthrough_send(handle, q, b"\x00")

        stats = engine.get_stats(handle)
