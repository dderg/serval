...
- **Known limit — cross-blend-boundary zero-pad (verified high-impact).** At every boundary between a plain Move and a QuinticBlendMove (in either direction), the shaper kernel sees a zero-pad discontinuity on the cross-boundary side. The discriminator (`_is_shape_bake_target` at `klippy/toolhead.py:350`) skips QBMs as neighbours for plain Moves, so a plain Move following a QBM gets `prev_unshaped=None`. On the upstream side, `CornerBlender._suppress_and_advance` (`klippy/blendplanner.py:774`) and `CornerBlender.flush` (`klippy/blendplanner.py:908-910`) pass `next_unshaped=None` whenever the following move isn't a blend. `CornerBlender._pending_prev` only ever holds prior-QBM payloads (lines 713-719) — never plain-Move payloads — so a QBM following a plain Move gets `prev_unshaped=None` too. Both directions zero-pad.
  
  An independent verifier confirmed this is **routine in normal prints**, not exceptional: blend drops on short-segment G-code (insufficient runway → `QuinticShape.from_moves` returns None at `klippy/blendquintic.py:609-611`), suppression rule fires (`should_suppress_quintic` at `klippy/blendmath.py:186`), pure-E retract/wipe moves (`klippy/blendplanner.py:680-681` flushes pending QBM with `next=None`), and **lazy-flush boundaries crossing a `LOOKAHEAD_FLUSH_TIME` cycle** (`klippy/blendplanner.py:908-910` strands any pending QBM at the drain edge with zero-pad on the next side, even though motion continues — this is a separate sub-gap not visible in the inner LookAheadQueue's deferred-last pattern, which only handles plain Moves). For a typical FDM print interleaving curved walls (blends) with infill crossovers, retracts, and seams (plain runs / pure-E), every transition is a zero-pad point — multiple times per layer.
  
  This means "shape-baked by construction" holds **within** blend runs and **within** plain-Move runs, but is degraded at every run-to-run transition. Not a correctness blocker for the immediate z_tilt straight-line target (no blend runs are involved in z_tilt's traverse pattern), but it materially limits the reach of A3's Pillar-1 claim for general printing. Architectural fix sketch: (a) unify the neighbour-source discriminator so QBMs can serve as neighbours for plain Moves (both classes already store `_unshaped_payload` with the same tuple layout); (b) extend `CornerBlender._pending_prev` to track the last-released plain-Move's unshaped payload, falling back to it when no prior QBM exists; (c) carry a `_pending_quintic_across_flush` slot at the `BlendPipelineLookAheadQueue` level mirroring the inner LookAheadQueue's `_pending_last`, so lazy-flush boundaries no longer strand QBMs with zero-pad. Approaches (a) + (b) close the within-flush boundaries; (c) closes the cross-flush gap. Track as Phase A6 or earlier — the hardware Trident test should be the first signal of priority.
