...
- **EtherCAT:** the endpoint, at the clean post-retract position, performs CiA-402 **homing method 35** ("current position is home"): set `6098h=35`, `607Ch = Z × counts_per_mm`, pulse the homing control-word, wait for "home attained", return to CSP. The drive re-zeros `6064h` into our frame.
  - **Software position limits (`607D`) are OPTIONAL / out of core scope.** They are net-new (the existing `set_drive_limits` writes *torque* `0x6072` + *following-error-window* `0x6065`, not `607D`). If wanted: `607D.01 = min × counts_per_mm`, `607D.02 = max × counts_per_mm`. The reporting fix does NOT need them.

**Post-merge integration (sota-motion already provides):** servo path in `_home_axis` (`klippy/extras/homing.py:270-327`) with `servo_handle`, `rail.get_homing_drive_limits()` (torque/ferr, `servo_axis.py:150`), `_run_servo_guarded_trip` (the torque-reduced trip), `_servo_drive_limits` ctx, and `bridge.set_drive_limits`/`restore_drive_limits` (→ `servo_torque`, SDO `0x6065`/`0x6072`). Reuse these; the new work plugs the method-35 framing + drive-framed reporting + overshoot retract into that path. **What's genuinely missing (all three):** method-35 framing (no `6098`/`607C` anywhere), overshoot-corrected retract, the post-retract uniform seed.
