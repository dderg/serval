...
`timer_read_time()` is **ISR-safe** here: on H7 it is `DWT->CYCCNT`, a single-cycle read. The "foreground-only" warning at `runtime_tick.c:71` applies to `runtime_host_now_us` (the wrapper that *divides* after the read), not the raw read; under `CONFIG_MCU_SIM` the software CYCCNT is also ISR-safe.
