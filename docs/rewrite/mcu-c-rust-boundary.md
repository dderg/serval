...
- State that must survive a reset has a third constraint beyond ownership and layout: **it must sit where nothing else runs first.** On H7 the bootloader at `CONFIG_FLASH_BOOT_ADDRESS` executes before `armcm_main` on every reset and stacks from the base of AXI SRAM, so the low ~2 KB of that region is scratch no matter what the linker script says about it. The DFU sentinel (PR #345/#346) was linked to `0x24000000` on exactly this reasoning — reserved by the linker, `NOLOAD`, never memset — and was still destroyed every boot, which cost the board its ability to self-flash. `.bkp_bss` and the tail of `.axi_bss` (`.axi_dfu_flag`) are the safe homes; the region base is not one.
...
| Runtime context backing (`rt_storage`) | C-declared `uint8_t` buffer in `src/runtime_storage.c`. H7: `.axi_bss` (AXI SRAM at 0x24000000) via cfg-gated section attribute. F4: regular `.bss` via default rule. Rust imports via `extern "C" { static rt_storage: UnsafeCell<[u8; RT_STORAGE_SIZE]>; }` and casts to `*mut RuntimeContext`. | Migrated 2026-05-19 from the `RT_CELL` Rust static with `#[link_section]`. `RT_STORAGE_SIZE` flows from Kconfig (`RUNTIME_STORAGE_SIZE`, per-MCU defaults: 120 KB on H7, 72 KB on F4) through both `src/runtime_storage.h` and `rust/runtime/build.rs`. The cargo-clean operational tripwire (`feedback_cargo_clean_between_mcus.md`) is now optional rather than safety-critical. |
| DFU reboot sentinel (`usb_boot_flag`, H7-only) | C static in `src/stm32/dfu_reboot.c`, tagged `section(".axi_dfu_flag")`. | Must survive `NVIC_SystemReset` so `dfu_reboot_check()` can divert the next boot into ROM DFU. `armcm_link.lds.S` emits `.axi_dfu_flag` after every other AXI static, clear of the bootloader stack at the region base — it landed at `0x24000000` when it was plain `.axi_bss` and was silently clobbered every boot, which broke self-flashing entirely. Add new reset-surviving state here or in `.bkp_bss`, never at the AXI base. |
...
- **2026-05-12 — F446 `configure_axes` crash.** F4 FPU disabled at boot (`SystemInit` skips CPACR when `__FPU_USED == 0`); Rust soft-float occasionally lowered reg-reg moves to `vmov`, which UNDEFINSTRs on M4F. **Resolution:** enable `CPACR.CP10/11` in `armcm_main` (`src/stm32/stm32f4.c`) when `__FPU_PRESENT == 1` (the original `CONFIG_KALICO_RUNTIME` gate was later dropped — the enable is unconditional on FPU-capable parts). **Rule reinforced:** B1 — boot / CPU-state setup is C's job; Rust assumes the CPU is configured before it runs.
