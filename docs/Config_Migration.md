...
| `high_precision_step_compress` (any section) | **Renamed.** Use `[mcu] stepcompress_encoder: hp` (the default) or `classic`; with `classic`, `stepcompress_max_error` bounds the error budget. |
