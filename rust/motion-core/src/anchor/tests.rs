...
fn a_stalled_first_commit_after_two_resumes_stays_a_continuation() {
    let mut a = Anchor::new();
    let (_, epoch) = a.anchor_segment(0.0, BENCH_SEG_SECS, 100.0);
    a.mark_parked();
    assert_eq!(epoch, StreamEpoch::Reposition);
    ...
    let resume_at = 108.1;
    let (_, epoch) = a.anchor_segment(2.0 * BENCH_SEG_SECS, 3.0 * BENCH_SEG_SECS, resume_at);
...
fn every_idle_resume_doubles_the_lead_the_next_timeline_starts_on() {
    ...
    let (t0, epoch) = a.anchor_segment(1.0, 2.0, 200.0);
    a.mark_parked();
    assert_eq!(epoch, StreamEpoch::Reanchor);
...
fn the_earned_lead_stops_at_the_pumps_push_horizon() {
    ...
    let mut t = 100.0;
    let mut seg = 0.0;
    let mut t0 = a.anchor_segment(seg, seg + 1.0, t).0;
    a.mark_parked();
    for _ in 0..8 {
        seg += 1.0;
        t += 1000.0;
        t0 = a.anchor_segment(seg, seg + 1.0, t).0;
        a.mark_parked();
    }
    assert!(
...
fn the_commit_right_behind_a_resume_does_not_release_the_earned_lead() {
    ...
    a.anchor_segment(0.0, 1.0, 100.0);
    a.mark_parked();
    a.anchor_segment(1.0, 1.05, 200.0);
    ...
    let (_, epoch) = a.anchor_segment(1.05, 1.1, 200.005);
    assert_eq!(epoch, StreamEpoch::Continuation);
