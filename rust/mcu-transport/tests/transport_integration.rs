...
use mcu_transport::{
    ConnectionState, EpochChange, IdentifyResponse, McuTransport, MessageKind, MockConnection,
    Transport, TransportError, bootstrap::encode_identify_response,
};
...
fn bootstrap_handshake_success() {
    ...
    let transport = McuTransport::with_schema_hash(host_half, host_hash, 0x01);
