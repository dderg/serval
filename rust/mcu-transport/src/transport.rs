...
use mcu_protocol::{MessageKind, PROTO_VERSION};
...
pub struct McuTransport<C: Connection> {
    inner: Arc<Inner<C>>,
}

impl<C: Connection> std::fmt::Debug for McuTransport<C> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("McuTransport")
            .field("state", &*self.inner.state.lock().unwrap())
            .finish()
    }
}

impl<C: Connection + 'static> McuTransport<C> {
    pub fn new(conn: C) -> Self {
        Self::with_schema_hash(conn, mcu_protocol::SCHEMA_HASH, PROTO_VERSION)
    }
...
impl<C: Connection + 'static> Transport for McuTransport<C> {
    fn call(
        &self,
        msg_type: MessageKind,
        body: &[u8],
