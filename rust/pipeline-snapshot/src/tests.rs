...
#[test]
fn clothoid_corner_dip_is_0863_of_formula_scv() {
    let (accel, deviation, v_wall) = (50_000.0, 0.05, 150.0);
    let snap = pipeline_snapshot(
        &[
            (0.0, 0.0, 0.2, 0.0, v_wall, accel),
            (60.0, 0.0, 0.2, 2.4, v_wall, accel),
            (60.0, 60.0, 0.2, 4.8, v_wall, accel),
        ],
        SnapshotParams {
            max_velocity: 2800.0,
            max_accel: accel,
            square_corner_velocity: 5.0,
            corner_deviation: Some(deviation),
            max_jerk: 2e7,
            max_extrude_only_velocity: None,
            max_extrude_only_accel: None,
            max_path_deviation: None,
    ...
    .unwrap();
    let eval_v = |pieces: &[Vec<f64>], t: f64| -> f64 {
        let row = pieces
            .iter()
            .find(|r| t >= r[0] && t <= r[1])
            .expect("t inside trajectory");
    ...
    while t <= snap.traj_t_end {
        let vx: f64 = eval_v(&snap.traj_x_pieces, t);
        let vy: f64 = eval_v(&snap.traj_y_pieces, t);
        profile.push(libm::hypot(vx, vy));
        t += 2e-5;
    }
    let first = profile
        .iter()
        .position(|&v| v >= 0.99 * v_wall)
        .expect("reaches cruise");
    let last = profile
        .iter()
        .rposition(|&v| v >= 0.99 * v_wall)
        .expect("returns to cruise");
    let v_min = profile[first..=last]
        .iter()
        .copied()
        .fold(f64::MAX, f64::min);
    let scv = (deviation * accel / (std::f64::consts::SQRT_2 - 1.0)).sqrt();
    let ratio = v_min / scv;
    assert!(
        (ratio - 0.863).abs() < 0.005,
        "clothoid dip ratio drifted: {ratio:.4} (update pa_test.py's CORNER_DIP_FACTOR)"
    );
}
