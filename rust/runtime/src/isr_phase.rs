...
pub(crate) const RT_PHASE_ARM: u32 = 6;
pub(crate) const RT_PHASE_CLENSHAW: u32 = 7;
#[allow(dead_code)]
...
#[inline]
pub(crate) fn arm_account(cycles: u32) {
    // The C accounting slot keeps its historical `monomial` symbol (dump-format
    // stability); it now measures the piece-arm (Chebyshev derivative) phase.
    #[cfg(not(any(test, feature = "host")))]
