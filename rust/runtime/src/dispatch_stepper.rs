...
// Diagnostic-only: stash the per-sample inputs of a StepsPerSampleExceeded fault
// into the survivable diag ring just before raising it. Tag 8 (DIAG_EV_RUST_FAULT)
// freezes the ring, so the two context entries (tags 9/10) pushed first are
// preserved through the shutdown-idle loop. Discriminates an eval/ring bad-curve
// value from a stale prev-step state bug.
#[cfg(any(not(any(test, feature = "host")), feature = "mcu-linux"))]
unsafe extern "C" {
...
fn capture_steps_fault_context(
    ...
    unsafe {
        ...
        diag_ring_push(9, p_end.to_bits(), p_sample_start.to_bits());
        diag_ring_push(10, prev_step_count as u32, target_step_count as u32);
        diag_ring_push(8, code, detail);
    }
