...
/// Foreground-written, ISR-read excitation request. Only `seq` plus the six
/// scalar parameters cross the task/ISR boundary, all as atomics (boundary
/// rule B5). `seq` is bumped last with `Release`; the ISR loads it first with
/// `Acquire`, so observing a new `seq` guarantees every parameter write is
/// visible — a seqlock-lite handshake with no torn reads.
#[derive(Debug)]
pub struct BuzzControl {
    seq: AtomicU32,
    axis_mask: AtomicU8,
    sign_mask: AtomicU8,
    freq_millihz: AtomicU32,
...
impl Buzz {
    ...
    pub fn arm(
        ...
        if !disarm {
            ...
            if freq_millihz == 0 || freq_millihz > MAX_FREQ_MILLIHZ {
                return -1;
            }
        ...
        c.freq_millihz.store(freq_millihz, Ordering::Relaxed);
    ...
    fn latch(&mut self, seq: u32, sample_rate_hz: f32) {
        ...
        let freq_millihz = c.freq_millihz.load(Ordering::Relaxed);
        ...
        if axis_mask == 0 || amplitude_nm == 0 || total_ticks < 2 || sample_rate_hz <= 0.0 {
            self.active = false;
            self.params = BuzzParams::idle();
            return;
        }

        let freq_hz = freq_millihz as f32 * 1.0e-3;
        ...
        self.params = BuzzParams {
            omega_per_tick: TWO_PI * freq_hz / sample_rate_hz,
            omega_per_sec: TWO_PI * freq_hz,
            amp_mm,
            total_ticks,
            ramp_ticks: ramp,
        };
    ...
    pub fn sample(&self, axis_idx: usize) -> BuzzSample {
        ...
        BuzzSample {
            ...
            velocity: env * amp * self.params.omega_per_sec * cos,
        }
    }
