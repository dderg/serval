...
/// The profile's boundary states must be the states its own move bodies end
/// at: a streaming cut carries `boundaries[cut]` as the next window's entry,
/// so a boundary below the reconstructed exit would emit a body ending at one
/// speed and re-plan the suffix from another. The clamp that once tied
/// boundaries to the seam plan hid exactly that on a fast travel into a slow
/// print run, where retreat pollution drags the seam plan to the brake ramp's
/// minimum while the reconstruction correctly rides the print ceiling.
#[test]
fn boundaries_match_reconstructed_exits() {
    for n in [7usize, 20, 200] {
        let (tx, _rx) = unbounded();
        let mut planner = Planner::new(config(4096));
        for m in &travel_then_print(n) {
            assert!(planner.feed(StreamInput::Move(m.clone()), &tx), "closed");
            planner.moves_since_plan = 0;
        }
        let prof = planner.plan();
        for (i, mv) in prof.moves.iter().enumerate() {
            let bnd = prof.boundaries[i + 1].v;
            assert!(
                (bnd - mv.exit_v).abs() <= 1.0e-6 * mv.exit_v.max(1.0),
                "n={n}: boundary {i} carries v={bnd} but the reconstructed body \
                 ends at {}",
                mv.exit_v
            );
        }
