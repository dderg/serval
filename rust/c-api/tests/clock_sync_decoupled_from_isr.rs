...
fn clock_sync_returns_widened_host_clock_not_seqlock() {
    let rt = c_api::runtime_handle_create();
    assert!(!rt.is_null());

    let mut mcu_clock: u64 = 0;
    let r = unsafe { c_api::runtime_clock_sync_request(rt, 42, 0, 0, &mut mcu_clock) };
    ...
    let r2 = unsafe { c_api::runtime_clock_sync_request(rt, 43, 0, 0, &mut mcu_clock) };
    assert_eq!(r2, 0);
