...
fn write_and_commit_correction_roundtrip() {
    ...
    unsafe {
        let rc = c_api::runtime_write_correction_piece(handle, 2, 0, 0, piece.as_ptr());
        assert_eq!(rc, c_api::RUNTIME_OK, "write failed: {rc}");
        let rc = c_api::runtime_commit_correction(handle, 2, 1, 1);
        assert_eq!(rc, c_api::RUNTIME_OK, "commit failed: {rc}");
    }
}
...
fn commit_correction_rejects_when_axis_busy() {
    ...
    unsafe {
        let rc = c_api::runtime_write_piece(handle, 2, 0, 0, piece.as_ptr());
        assert_eq!(rc, c_api::RUNTIME_OK);
        let rc = c_api::runtime_commit_head(handle, 2, 1);
        assert_eq!(rc, c_api::RUNTIME_OK);

        let rc = c_api::runtime_write_correction_piece(handle, 2, 0, 0, piece.as_ptr());
        assert_eq!(rc, c_api::RUNTIME_OK);
        let rc = c_api::runtime_commit_correction(handle, 2, 1, 1);
        assert_eq!(rc, runtime::error::RUNTIME_ERR_MOTION_IN_PROGRESS);

        c_api::runtime_discard_pending(handle);
    }
}
...
fn normal_commit_rejects_when_correction_active() {
    ...
    unsafe {
        ...
        assert_eq!(rc, c_api::RUNTIME_OK);
        let rc = c_api::runtime_commit_head(handle, 2, 1);
        assert_eq!(rc, runtime::error::RUNTIME_ERR_CORRECTION_IN_PROGRESS);
...
fn correction_ffi_null_rt_is_null_ptr_error() {
    ...
    unsafe {
        let rc =
            c_api::runtime_write_correction_piece(core::ptr::null_mut(), 2, 0, 0, piece.as_ptr());
        assert_eq!(rc, c_api::RUNTIME_ERR_NULL_PTR);
        let rc = c_api::runtime_commit_correction(core::ptr::null_mut(), 2, 1, 1);
        assert_eq!(rc, c_api::RUNTIME_ERR_NULL_PTR);
    }
}
