...
fn subsystem_mapping() {
    assert_eq!(subsystem_for_target("motion_engine::bridge"), "bridge");
    assert_eq!(subsystem_for_target("motion_engine::planner"), "motion");
    assert_eq!(
        subsystem_for_target("host_rt::host_io::reactor"),
        "mcu-comms"
    );
    assert_eq!(subsystem_for_target("motion_engine::homing"), "homing");
