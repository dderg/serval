...
use super::{NudgeStreamState, nudge_anchor_base};

#[test]
fn consecutive_nudges_get_monotonic_non_overlapping_anchors() {
    let lead = 0.25;
    let base1 = nudge_anchor_base(0.0, 100.0, lead);
...
#[test]
fn nudge_stream_state_accumulates_within_and_across_nudges() {
    let lead = 1.0;
    let mut st = NudgeStreamState::new();

    // First nudge: 3 contiguous segments [0,0.05], [0.05,0.10], [0.10,0.15].
    let (t0_a, fresh_a) = st.anchor(0.0, 0.05, 100.0, lead);
    assert!(fresh_a, "first segment of first nudge must be fresh");
    let (t0_b, fresh_b) = st.anchor(0.05, 0.10, 100.0, lead);
    assert!(!fresh_b, "second segment of same nudge must not be fresh");
    assert_eq!(t0_a, t0_b, "same nudge: t0 must be stable across segments");
    let (t0_c, fresh_c) = st.anchor(0.10, 0.15, 100.0, lead);
    assert!(!fresh_c);
    assert_eq!(t0_a, t0_c);
    ...
    let (t0_n3, fresh_n3) = st.anchor(0.0, 0.05, 300.0, lead);
    assert!(fresh_n3);
    assert!(
        (t0_n3 - (300.0 + lead)).abs() < 1e-9,
        "after real gap, t0 anchors at host_now+lead, got {t0_n3}"
    );
}
