...
#[test]
fn peek_across_piece_boundary_does_not_retire() {
    let mut ring = AxisRing::new();
    let t0: u64 = 1_000_000_000;
    let dur_ns: u64 = 1_000_000;
    ring.push_entry(linear_entry(0.0, 10.0, t0, 0.001)).unwrap();
    ring.push_entry(linear_entry(10.0, 30.0, t0 + dur_ns, 0.001))
        .unwrap();
...
#[test]
fn peek_in_gap_and_past_stream_end_is_stationary() {
    let mut ring = AxisRing::new();
    let t0: u64 = 1_000_000_000;
    let dur_ns: u64 = 1_000_000;
    ring.push_entry(linear_entry(0.0, 10.0, t0, 0.001)).unwrap();
    ring.push_entry(linear_entry(10.0, 20.0, t0 + 3 * dur_ns, 0.001))
        .unwrap();
