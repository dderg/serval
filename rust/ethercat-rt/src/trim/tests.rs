...
fn armed_bank(gain_micro: u32, clamp_um: u16, lpf_millihz: u32) -> DiffTrimBank {
    ...
    assert_eq!(bank.set(4, 0, 1, gain_micro, clamp_um, lpf_millihz), 0);
    bank
}
...
fn run_spring(
    ...
    for _ in 0..cycles {
        ...
        bank.update(&torques, &[streaming; 4], &mut offset);
    }
    (diff_tenths, offset[0])
}

#[test]
fn set_rejects_bad_slots_gain_clamp_and_lpf() {
    let mut bank = DiffTrimBank::new(CYCLE_NS);
    assert_eq!(bank.set(4, 2, 2, 50_000, 150, 25_000), ERR_TRIM_BAD_SLOT);
    assert_eq!(bank.set(4, 0, 4, 50_000, 150, 25_000), ERR_TRIM_BAD_SLOT);
    assert_eq!(
        bank.set(4, 0, 1, MAX_TRIM_GAIN_MICRO + 1, 150, 25_000),
        ERR_TRIM_BAD_GAIN
    );
    assert_eq!(bank.set(4, 0, 1, 50_000, 0, 25_000), ERR_TRIM_BAD_CLAMP);
    assert_eq!(
        bank.set(4, 0, 1, 50_000, MAX_TRIM_CLAMP_UM + 1, 25_000),
        ERR_TRIM_BAD_CLAMP
    );
    assert_eq!(bank.set(4, 0, 1, 50_000, 150, 500), ERR_TRIM_BAD_LPF);
    assert_eq!(
        bank.set(4, 0, 1, 50_000, 150, MAX_TRIM_LPF_MILLIHZ + 1),
        ERR_TRIM_BAD_LPF
    );
    assert!(!bank.active());
}
...
fn set_rejects_slot_shared_with_another_pair() {
    ...
    assert_eq!(bank.set(4, 1, 2, 50_000, 150, 25_000), ERR_TRIM_SLOT_IN_USE);
    assert_eq!(bank.set(4, 2, 3, 50_000, 150, 25_000), 0);
}
...
#[test]
fn frozen_pair_holds_its_offset_when_not_streaming() {
    let mut bank = armed_bank(50_000, 150, 25_000);
    ...
    assert_eq!(offset[0], settled, "idle cycles must not move the offset");
}
...
#[test]
fn reset_clears_offset_filter_and_warning_state() {
    let mut bank = armed_bank(2_000_000, 50, 25_000);
    let mut offset = vec![0f64; 4];
    for _ in 0..8_000 {
        offset.iter_mut().for_each(|o| *o = 0.0);
...
fn replacing_a_pair_restarts_from_zero_offset() {
    ...
    assert_eq!(bank.set(4, 1, 0, 100_000, 150, 25_000), 0);
    let mut offset = vec![0f64; 4];
    bank.update(&[0.0, 0.0, 0.0, 0.0], &[true; 4], &mut offset);
    assert_eq!(offset[0], 0.0);
}
