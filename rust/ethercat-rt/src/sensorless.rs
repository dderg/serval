...
impl SensorlessArm {
    fn new(endstop_id: u8, torque_trip_tenth_pct: u16) -> Self {
        Self {
            endstop_id,
...
impl SensorlessBank {
    ...
    pub fn arm(&mut self, slot: usize, endstop_id: u8, torque_trip_tenth_pct: u16) {
        self.arms[slot] = Some(SensorlessArm::new(endstop_id, torque_trip_tenth_pct));
    }
    ...
    /// Polls every armed slot against `torque_of(slot)` and calls
