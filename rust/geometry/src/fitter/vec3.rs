...
pub(super) fn madd(p: [f64; 3], s: f64, d: [f64; 3]) -> [f64; 3] {
...
pub(super) fn signed_angle(from: [f64; 3], to: [f64; 3], plane_normal: [f64; 3]) -> f64 {
    dot(cross(from, to), plane_normal).atan2(dot(from, to))
}

pub(super) fn turn_normal(t_in: [f64; 3], t_out: [f64; 3]) -> Option<[f64; 3]> {
    let d = dot(t_out, t_in);
    let perp = sub(t_out, scale(t_in, d));
    let n = norm(perp);
    if n < super::TURN_NORMAL_EPS {
        None
    } else {
