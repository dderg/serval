...
#[test]
fn kappa_is_read_from_the_turning_signal() {
    let verts: Vec<[f64; 3]> = (0..=6)
        .map(|k| {
            let phi = (k as f64 / 6.0) * (PI / 2.0);
            [3.0 * phi.sin(), 3.0 - 3.0 * phi.cos(), 0.0]
        })
        .collect();
    let plane = [0.0, 0.0, 1.0];
    let (s, theta) = turning_signal(&verts, plane);
    let k1 = leg_kappa(&s, &theta, 1);
    let k3 = leg_kappa(&s, &theta, 3);
    assert!((k1 - 1.0 / 3.0).abs() < 0.05, "k1 {k1}");
    assert!(
        (k1 - k3).abs() < 1e-6,
        "uniform arc has constant kappa: {k1} {k3}"
    );
}
