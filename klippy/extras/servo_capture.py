...
class ServoCapture:
    ...
    def _resolve_node(self, gcmd):
        ...
        return self._node_drives(motors)

    def _node_drives(self, motors):
    ...
    def _resolve_servos(self, servos):
        from . import servo_axis
        ...
        for token in servos:
            ...
            motors.append(motor)
        if not motors:
            raise self.printer.command_error("SERVO_CAPTURE: no servos named")
        return self._node_drives(motors)
    ...
    def _begin(self, node, drives, path):
        ...
        try:
            ...
            raise self.printer.command_error(
                "SERVO_CAPTURE: start failed: %s" % (e,)
            )
    ...
    def _end(self):
        if self.active is None:
            raise self.printer.command_error("SERVO_CAPTURE: no capture active")
        ...
        return path, samples, node.get_cycle_us()

    def start_capture_to(self, path, servos):
        """Start a capture of the named servos writing to an explicit path.
        The calibration extra uses this with run-directory paths; the gcode
        commands keep the flat single-file behavior for manual use."""
        node, drives = self._resolve_servos(servos)
        os.makedirs(os.path.dirname(path), exist_ok=True)
        return self._begin(node, drives, path)

    def stop_capture(self):
        """Stop the active capture, returning (path, samples, cycle_us).
    ...
    def cmd_SERVO_CAPTURE_STOP(self, gcmd):
        path, samples, cycle_us = self._end()
