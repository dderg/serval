...
class ResonanceTester:
    ...
    def _submit_buzz(
        ...
        # Amplitude is the displacement at freq_start; the MCU tapers it as
        # f_start/f(t) over a sweep, holding peak velocity constant and peak
        # accel proportional to f. The accel-safety clamp therefore binds at the
        # HIGHEST frequency in the run, where peak accel = accel_per_hz * f_hi.
        f_hi = max(freq_start, freq_end)
        max_peak_accel = 15000.0  # mm/s^2, ~1.5 g
        if amplitude_mm <= 0.0:
            if accel_per_hz * f_hi > max_peak_accel:
                accel_per_hz = max_peak_accel / f_hi
                gcmd.respond_info(
                    "RESONANCE_BUZZ: clamped accel_per_hz to %.1f to keep peak "
                    "accel <= %.0f mm/s^2 at %.1f Hz"
                    % (accel_per_hz, max_peak_accel, f_hi)
                )
            amplitude_mm = accel_per_hz / (4.0 * math.pi**2 * freq_start)
        max_amplitude_mm = 5.0
        if amplitude_mm > max_amplitude_mm:
            raise gcmd.error(
                "RESONANCE_BUZZ amplitude %.3f mm at %.1f Hz exceeds %.1f mm "
                "ceiling" % (amplitude_mm, freq_start, max_amplitude_mm)
            )

        toolhead.wait_moves()
        # No phase-stepping handover: the MCU routes the buzz by each axis's step
        # mode — a phase-mode motor stays in phase stepping and is driven by XDIRECT
        # coil updates, a pulse-mode motor by STEP/DIR edges — both off the same
        # excitation curve. wait_moves() above guarantees the motors are idle.
    ...
    def cmd_RESONANCE_BUZZ_SWEEP(self, gcmd):
        ...
        accel_per_hz = gcmd.get_float("ACCEL_PER_HZ", 75.0, above=0.0)
        amplitude_mm = gcmd.get_float("AMPLITUDE", 0.0, minval=0.0)
        # Either a total DURATION, or a sweep rate HZ_PER_SEC (duration derived).
        ...
        # One ramp for the whole chirp; a few periods of the lowest frequency,
        # capped well under the sweep length so it stays amortized.
        ramp = gcmd.get_float(
            "RAMP",
            min(duration * 0.1, 3.0 / min(freq_start, freq_end)),
            above=0.0,
        )
        self._submit_buzz(
            gcmd,
