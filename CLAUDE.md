...
- **Motion planner entry point:** `setup_pipeline` in `rust/motion-engine/src/stream.rs` wires the streaming stages (fitter → planner → lowerer → shaper) — read it first when exploring the planner.
